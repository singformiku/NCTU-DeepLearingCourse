...
print(args.vgg_model)
G.to(device)
D.to(device)
VGG.to(device)
G.train()
D.train()
G.load_state_dict(torch.load('./your_project_name_results/generator_param.pkl'))
D.load_state_dict(torch.load('./your_project_name_results/discriminator_param.pkl'))
...
for epoch in range(args.train_epoch):
    ...
    with torch.no_grad():
        ...
        for n, (x, _) in enumerate(test_loader_src):
            ...
            if n == 4:
                break
        torch.save(G.state_dict(), os.path.join(args.name + '_results', str(epoch+1) +'generator_latest.pkl'))

        torch.save(D.state_dict(), os.path.join(args.name + '_results', str(epoch+1) +'discriminator_latest.pkl'))
