...
public:
		...
		// for (state* move = after; move != after + 4; move++) {
			// if (move->assign(b)) {
				...
				// if (move->value() > best->value()){
					// best = move;
                    // //======My implement=========================
                    // board tmp = best->after_state();
                    // float val_2=0, val_4=0, expectation=0;//get the value of 2048 board
                    // int count = 0;
                    // for(int i=0;i<16;i++){  
                        // if(tmp.at(i)==0){
                            // count++;
                            // tmp.set(i, 1);//assign 2^1 to estimate
                            // val_2+=estimate(tmp);
                            // tmp.set(i, 2);//assign 2^2 to estimate
                            // val_4+=estimate(tmp);
                            // tmp.set(i, 0);//restore the value
                        // }
                    // }
                    // expectation=(0.1*val_4+0.9*val_2)/count;
                    // best->set_value(expectation+best->reward());                    
                    // //======My implement=========================
                // }
			// } else {
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				expectation=(0.1*val_4+0.9*val_2)/count;
				move->set_value(expectation+best->reward());  
				if (move->value() > best->value())
				best = move;
				//======My implement=========================
    ...
    // void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		// for (path.pop_back() /* terminal state */; path.size();) {
            ...
            // state& move_2 = path.back();
            // // move->set_value(move->reward() + estimate(move->after_state()));
			// float error = exact - (move_2.value() - move_2.reward());
            // exact = move_2.reward() + update(move.before_state(), alpha * error);
		// }
	// }
    //======================================================Backup=======================================================================
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
...
int main(int argc, const char* argv[]) {
	...
	path.reserve(20000);
    std::ofstream myfile;
    myfile.open ("reward.csv", std::ios::trunc);
	for (size_t n = 1; n <= total; n++) {
		...
		while (true) {
            ...
            // debug <<"==================="<< b.at(15)<<std::endl;
			state best = tdl.select_best_move(b);
			path.push_back(best);//before <<<===========================
			...
			if (best.is_valid()) {
				...
				b = best.after_state();
				b.popup();
                // path.push_back(b);//after
			} else {
                // path.push_back(best);//after
				break;
			}
		}
        // if(n%10)
		...
		path.clear();
        if(n%10000==0){
            std::cout<<"===========saving"<<n<<"episode========\n";
            tdl.save("MikuFinalVer3.0");
        }
	}
